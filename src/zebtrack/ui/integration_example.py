...
class IntegrationExample:
    ...
    def _example_3_control_panel(self):
        ...
        # Subscrever eventos do componente
        if self.event_bus:
            self.event_bus.subscribe("control.start_recording", 
                                     self._on_start_recording)
            self.event_bus.subscribe("control.stop_recording", 
                                     self._on_stop_recording)
            self.event_bus.subscribe("control.process_video", 
        ...
        ttk.Label(
            tab,
            text="ControlPanelWidget centraliza controles de gravação",
            padding=10
        ).pack()
    ...
    def _example_4_combined_tab(self):
        ...
        self.zone_controls_combined = ZoneControlsWidget(
            left_panel,
            event_bus=self.event_bus
        )
...
def main():
    ...
    # Criar exemplo
    app = IntegrationExample(root, controller, event_bus)
