...
class ProjectOverviewWidget(BaseWidget):
    ...
    def _build_video_tree(self) -> None:
        ...
        scrollbar = create_scrollbar(
            tree_container,
            orient="vertical",
            command=self.project_overview_tree.yview
        )
    ...
    def _on_video_selected(self, event) -> None:
        ...
        if selection:
            item_id = selection[0]
            self.emit_event("project.video_selected", {
                "item_id": item_id
            })
    ...
    def _on_video_double_click(self, event) -> None:
        ...
        if selection:
            item_id = selection[0]
            self.emit_event("project.video_double_click", {
                "item_id": item_id
            })
    ...
    def _on_video_right_click(self, event) -> None:
        ...
        if item_id:
            ...
            self.emit_event("project.video_right_click", {
                "item_id": item_id,
                "x": event.x_root,
                "y": event.y_root
            })
