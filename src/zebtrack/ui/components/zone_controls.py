...
class ZoneControlsWidget(BaseWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.controls_canvas.create_window(
            (0, 0),
            window=self.zone_controls_frame,
            anchor="nw"
        )
    ...
    def _build_drawing_actions(self) -> None:
        ...
        ttk.Entry(
            stabilization_frame,
            textvariable=self.stabilization_frames_var,
            width=5
        ).pack(side="left")
    ...
    def _build_template_section(self) -> None:
        ...
        ttk.Button(
            template_selector,
            text="Aplicar",
            command=self._on_apply_template_clicked
    ...
    def _build_video_selector(self) -> None:
        ...
        self.video_search_var.trace_add(
            "write",
            lambda *_: self._on_video_search_changed()
        )
        ttk.Entry(
            search_frame,
        ...
        ttk.Button(
            ...
            text="🔄",
            width=3,
            command=self._on_video_refresh_clicked
        ).pack(side="left")
    ...
    def _build_zone_list(self) -> None:
        ...
        scrollbar = create_scrollbar(
            zone_list_frame,
            orient="vertical",
            command=self.zone_listbox.yview
        )
    ...
    def _build_roi_inclusion_panel(self) -> None:
        ...
        ttk.Entry(
            self.radius_frame,
            textvariable=self.roi_buffer_radius_var,
            width=10
        ...
        ttk.Entry(
            self.overlap_frame,
            textvariable=self.roi_overlap_ratio_var,
            width=10
        ).pack(side="left", padx=(0, 10))
        ttk.Label(
            self.overlap_frame,
    ...
    def _on_apply_template_clicked(self) -> None:
        ...
        self.emit_event("zone.template_apply", {
    ...
    def _on_video_search_changed(self) -> None:
        ...
        self.emit_event("zone.video_search_changed", {
    ...
    def _on_video_tree_double_click(self, event) -> None:
        ...
        if selection:
            item_id = selection[0]
            self.emit_event("zone.video_double_click", {
                "item_id": item_id
            })
    ...
    def _on_load_video_frame_clicked(self) -> None:
        ...
        if selection:
            item_id = selection[0]
            self.emit_event("zone.video_frame_load", {
                "item_id": item_id
            })
    ...
    def _on_zone_right_click(self, event) -> None:
        ...
        if selection:
            item_id = selection[0]
            self.emit_event("zone.list_item_right_click", {
                "item_id": item_id,
                "x": event.x_root,
                "y": event.y_root
            })
    ...
    def _on_zone_double_click(self, event) -> None:
        ...
        if selection:
            item_id = selection[0]
            self.emit_event("zone.list_item_double_click", {
                "item_id": item_id
            })
    ...
    def show_single_analysis_options(self) -> None:
        ...
        if hasattr(self, 'single_analysis_options_frame'):
            self.single_analysis_options_frame.pack(fill="x", pady=5, before=self.zone_controls_frame.winfo_children()[1])
