...
class ApplicationGUI:
    ...
    - Construir a janela principal e as visualizações da interface (boas-vindas
      e controle principal).
    - Inicializar e gerenciar os módulos de backend como câmera, detector e
      gravador.
    - Manter o estado da aplicação através de variáveis de controle.
    - Orquestrar as threads para captura de quadros, detecção de objetos e
      gravação de vídeo para garantir que a interface do usuário permaneça
      responsiva.
    """
    ...
    def __init__(self, root):
        ...
        self.camera = (
            None  # Câmera será inicializada apenas se um projeto 'live' for criado.
        )
        self.arduino = Arduino()
        self.detector = (
            None  # Detector será inicializado após o carregamento de um projeto.
        )
        ...
        self.is_processing = (
            True  # Flag para habilitar/desabilitar a detecção de objetos.
        )
        self.is_capturing_for_video = (
            False  # Flag para indicar se os quadros devem ser salvos em um vídeo.
        )
        self.is_recording = False  # Flag mestre para indicar se os dados (CSV, vídeo)
        # devem ser gravados.
        self.active_frame_source = (
            None  # Armazena a fonte de quadros ativa (câmera ou arquivo de vídeo).
        )
        self.welcome_frame = None  # Contêiner para a tela de boas-vindas.
        self.main_controls_frame = (
            None  # Contêiner para os controles principais da aplicação.
        )
        self.currently_processing_video = (
            None  # Caminho para o arquivo de vídeo sendo processado no momento.
        )

        # --- Threads e Filas ---
        # Usado para executar tarefas demoradas (como processamento de vídeo)
        # sem bloquear a GUI.
        self.program_exit_event = (
            threading.Event()
        )  # Sinaliza para todas as threads que o programa está fechando.
        self.video_stop_event = (
            threading.Event()
        )  # Sinaliza para a thread de gravação de vídeo parar.
        self.frame_queue = queue.Queue(
            maxsize=10
        )  # Fila para passar quadros da thread de captura para a de detecção.
        self.video_queue = queue.Queue(
            maxsize=300
        )  # Fila para passar quadros para a thread de gravação de vídeo.
    ...
    def _create_main_control_frame(self):
        ...
        if project_type == "live":
            ...
            self.process_video_btn = Button(
                ...
                command=self._process_next_video,
            )
        ...
        Button(
            self.main_controls_frame, text="Close Project", command=self._close_project
        ).pack(side="left", padx=5)

        status_text = (
            f"Project: {self.project_manager.get_project_name()} ({project_type})"
        )
    ...
    def _load_project_view(self):
        ...
        if project_type == "live":
            ...
            try:
                ...
                self.detector.update_scaling(
                    self.camera.actual_width, self.camera.actual_height
                )
            except IOError as e:
            ...
            if next_video is None:
                ...
                self.status_var.set(
                    f"Project: {self.project_manager.get_project_name()} - "
    ...
    def _file_processing_loop(self):
        ...
        """
        if not self.is_recording or not isinstance(
            self.active_frame_source, VideoFileSource
        ):
        ...
        # The value from the Entry widget is a string, so it must be converted
        # to an integer. Validation is handled before this thread starts, so we
        # can safely convert.
        try:
            processing_interval = int(self.processing_interval_var.get())
        except ValueError:
            # Fallback in case of an unlikely error, though validation should
            # prevent this.
        ...
        while not self.program_exit_event.is_set() and frame_number < total_frames:
            ...
            if target_frame >= total_frames:
                logging.info(
                    f"Target frame {target_frame} exceeds total frames "
                    f"{total_frames}. Finishing."
                )
                break
            ...
            if not ret:
                logging.warning(
                    f"Could not read frame {target_frame} even though it's "
                    "within total_frames."
                )
                break

            # `get_current_frame_number` pode não ser exato após um `set`,
            # então usamos o target como referência
            frame_number = target_frame
            logging.info(f"Processing frame {frame_number}...")

            # Se a pré-visualização estiver desativada, atualiza a barra de
            # status com o progresso
            if not show_preview:
                if total_frames > 0:
    ...
    def _video_recording_loop(self):
        ...
        Esta função é usada apenas para projetos 'live' para salvar a gravação da
        câmera. Ela consome quadros da `video_queue` e os escreve no arquivo de
        vídeo usando o `recorder`.
        """
    ...
    def _create_project_workflow(self):
        ...
        base_path = filedialog.askdirectory(
            title="Select a Parent Folder for the Project"
        )
        if not base_path:
            return
        ...
        if success:
            logging.info(
                f"Successfully created project '{project_name}' at {project_path}"
            )
    ...
    def _open_project_workflow(self):
        """Abre um projeto existente a partir de um diretório selecionado."""
    ...
    def _close_project(self):
        ...
        if self.is_recording:
            ...
            # O cleanup ocorrerá, mas talvez não a atualização da UI, o que é
            # aceitável ao fechar.
    ...
    def _define_groups(self):
        ...
        group_count = simpledialog.askinteger(
    ...
    def _start_recording(self):
        ...
        def on_confirm():
            cobaia_number = simpledialog.askstring(
                "Cobaia Number", "Enter the cobaia number:"
            )
    ...
    def _process_next_video(self):
        """Inicia o fluxo de trabalho para processar o próximo vídeo."""
        if self.is_recording:
        ...
        def on_confirm():
            cobaia_number = simpledialog.askstring(
                "Cobaia Number", "Enter the cobaia number for this run:"
            )
            if not cobaia_number:
                return
            selection_window.destroy()
            ...
            try:
                ...
                if interval < 1:
                    messagebox.showwarning(
                        "Invalid Input", "Frame interval must be 1 or greater."
                    )
                    return
            except ValueError:
                messagebox.showerror(
                    "Invalid Input",
                    "Frame interval must be a valid number. "
                    f"You entered: '{self.processing_interval_var.get()}'",
                )
                return
            ...
            if success:
                ...
                logging.error(
                    f"Failed to start recorder for video processing: {video_path}"
                )
                messagebox.showerror(
    ...
    def _cleanup_after_processing(self):
        ...
        Este método é chamado a partir da thread de processamento via `root.after()`.
        """
        video_name = os.path.basename(self.currently_processing_video)
    ...
    def _handle_source_finished(self):
        ...
        """
        logging.warning(
            "'_handle_source_finished' was called. This should typically not "
            "happen for file processing anymore."
        )
        if self.is_recording:
            self.is_recording = False
            self.recorder.stop_recording()
