...
class Reporter:
    def __init__(self, behavioral_results: dict, roi_results: dict, metadata: dict, tracking_data: np.ndarray = None):
        self.behavioral_results = behavioral_results
        self.roi_results = roi_results
        self.metadata = metadata
        self.tracking_data = tracking_data if tracking_data is not None else self._generate_mock_tracking_data()
    ...
    def _flatten_roi_results(self) -> dict:
        ...
        for roi_name, metrics in self.roi_results.items():
            for metric_name, value in metrics.items():
                clean_metric_name = metric_name.replace("_roi", "").replace("_s", "").replace("_n", "")
    ...
    def _create_tidy_dataframe(self) -> pd.DataFrame:
        combined_data = {**self.metadata, **self.behavioral_results, **self._flatten_roi_results()}
    ...
    def export_summary_data(self, output_path: str, format: str = 'excel'):
        ...
        if format == 'excel':
            self.tidy_data.to_excel(f"{output_path}.xlsx", index=False, engine='openpyxl')
        elif format == 'csv':
    ...
    def generate_trajectory_plot(self, ax: plt.Axes = None) -> plt.Figure:
        ...
        ax.add_patch(patches.Rectangle((0, 0), 1024, 1024, fill=False, edgecolor='black', lw=2))
    ...
    def generate_heatmap(self, ax: plt.Axes = None) -> plt.Figure:
        ...
        ax.clear()
        x, y = self.tracking_data[:, 0], self.tracking_data[:, 1]
        heatmap, xedges, yedges = np.histogram2d(x, y, bins=50, range=[[0, 1024], [0, 1024]])
        heatmap = gaussian_filter(heatmap.T, sigma=2)
        im = ax.imshow(heatmap, cmap='hot', origin='lower', extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
    ...
    def export_individual_report(self, output_path: str):
        document = Document()
        document.add_heading(f"Relatório de Análise - {self.metadata.get('experiment_id', 'N/A')}", level=1)
        ...
        df = self.tidy_data.drop(columns=[k for k in self.metadata.keys() if k in self.tidy_data.columns])
        ...
        for _, row in df.iterrows():
            ...
            for i, value in enumerate(row):
                cells[i].text = f"{value:.2f}" if isinstance(value, (int, float)) else str(value)
        document.add_page_break()
        document.add_heading("Visualizações", level=2)
        for plot_func, name in [(self.generate_trajectory_plot, "Trajetória"), (self.generate_heatmap, "Mapa de Calor")]:
            fig = plot_func()
            memfile = io.BytesIO()
    ...
    @staticmethod
    def _generate_comparative_boxplot(df: pd.DataFrame, metric: str, title: str) -> plt.Figure:
    ...
    def export_project_report(aggregated_df: pd.DataFrame, output_path: str):
        ...
        desc_stats = aggregated_df.groupby('group_id')['distancia_total_cm'].agg(['mean', 'std', 'count'])
        document.add_paragraph("Estatísticas para a Distância Total Percorrida (cm):")
        table = document.add_table(rows=1, cols=len(desc_stats.columns) + 1)
        table.style = 'Table Grid'
        hdr_cells = table.rows[0].cells
        hdr_cells[0].text = 'group_id'
        for i, col_name in enumerate(desc_stats.columns): hdr_cells[i+1].text = col_name
        for index, row in desc_stats.iterrows():
            row_cells = table.add_row().cells
            row_cells[0].text = str(index)
            for i, value in enumerate(row): row_cells[i+1].text = f"{value:.2f}"
        document.add_page_break()
        document.add_heading("Gráficos Comparativos", level=2)
        boxplot_fig = Reporter._generate_comparative_boxplot(aggregated_df, 'distancia_total_cm', 'Comparação da Distância Total Percorrida por Grupo')
        memfile = io.BytesIO()
