...
def export_project_report(
    ...
    descriptive_stats_df: pd.DataFrame | None = None,
) -> None:
    ...
    Args:
        ...
        descriptive_stats_df: Optional pre-computed descriptive statistics table.
    """
    from zebtrack import __version__
    ...
    document.add_paragraph(
        f"Generated by DRerio LogAI v{__version__} on {datetime.now():%Y-%m-%d %H:%M}"
    )
    ...
    _render_comparative_boxplots(document, aggregated_df)

    # Descriptive statistics pivot table (if provided by unified report mixin)
    if descriptive_stats_df is not None and not descriptive_stats_df.empty:
        _render_descriptive_stats_table(document, descriptive_stats_df)
...
def _render_descriptive_stats_table(document: Any, descriptive_stats_df: pd.DataFrame) -> None:
    ...
    cols_to_show = list(descriptive_stats_df.columns[:20])
    table = document.add_table(rows=1, cols=len(cols_to_show) + 1)
    table.style = "Table Grid"
    hdr = table.rows[0].cells
    hdr[0].text = "Group / Day"
    for i, col_name in enumerate(cols_to_show):
        hdr[i + 1].text = col_name

    for idx, row in descriptive_stats_df[cols_to_show].iterrows():
        cells = table.add_row().cells
        cells[0].text = str(idx)
        for i, val in enumerate(row):
            cells[i + 1].text = f"{val:.2f}" if isinstance(val, float) else str(val)


def _render_comparative_boxplots(document: Any, aggregated_df: pd.DataFrame) -> None:
    """Render comparative boxplot figures into a Word document."""
    document.add_page_break()
