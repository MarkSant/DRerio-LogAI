...
class AquariumDetector:
    ...
    def __init__(self, model_path: str):
        ...
        try:
            ...
            log.error("aquarium_detector.init.failed", model_path=model_path, error=str(e))
            raise
    ...
    def detect_aquariums(self, video_path: str, stabilization_frames: int = 30) -> list:
        ...
        try:
            ...
            if not source.is_opened():
                log.error("aquarium_detector.detect.video_open_failed", video_path=video_path)
                return []
            ...
            for i in range(stabilization_frames):
                ...
                if results and results[0].masks and results[0].masks.xy:
                    ...
                    log.info("aquarium_detector.detect.found_polygons", frame=i, count=len(polygons))

            if detected_polygons:
                log.info("aquarium_detector.detect.finished", count=len(detected_polygons))
