...
class AppController:
    ...
    def _get_reporter_for_selected_experiment(self) -> Reporter | None:
        ...
        if not exp_id or exp_id not in self.report_results_paths:
            self.view.show_warning("Seleção Inválida", "Selecione um experimento.")
            return None
        ...
        try:
            ...
            b_results = {
                ...
                if k in BehavioralAnalyzer.METRIC_KEYS
            }
            ...
            self.view.show_error("Erro ao Carregar", f"Falha ao carregar dados: {e}")
            return None
