...
class OpenVINOPlugin(DetectorPlugin):
    ...
    def __init__(self, model_path: str, expected_hash: str | None = None):
        ...
        self.association_tolerance = (
            settings.video_processing.tracker_association_tolerance
        )
        self.tracker = Sort(
        ...
        if tracked_objects is not None and len(tracked_objects) > 0:
            for x1, y1, x2, y2, track_id in tracked_objects:
                ...
                for det in dets_for_sort:
                    if (
                        abs(det[0] - x1) < self.association_tolerance
                        and abs(det[1] - y1) < self.association_tolerance
                        and abs(det[2] - x2) < self.association_tolerance
                        and abs(det[3] - y2) < self.association_tolerance
                    ):
                        conf = det[4]
                        break
