...
list in the same PR that migrates a package. When a whole package is done, its
per-file entries collapse into the package path, so a new file under it is
covered the moment it is added. The migration ends when the list is just
`("src/zebtrack",)`.

**The scanner's count is a floor, not a ceiling.** It only detects accented
characters, so unaccented Portuguese — `Salvar projeto`, `Nenhum video`,
`Remover`, `dias`, `grupos` — is invisible to it. In practice each batch turned
up between a third and half again as many literals as the scan reported. Read the
whole file, and grep for `text=`, `label=`, `title=`, `show_*`, `set_status` and
`add_command` to catch the rest.
