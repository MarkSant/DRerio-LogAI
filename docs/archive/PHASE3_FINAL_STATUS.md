...
1. `_change_roi_color` → DialogManager (35 lines)
2. `_rename_selected_roi` → DialogManager (29 lines)
3. `_create_drawing_buttons` → WidgetFactory (25 lines)
4. `_load_selected_video_frame` → CanvasManager (35 lines)
5. `_on_import_roi_template` → WidgetFactory (25 lines)

### Third Part (3 methods - 103 lines)

1. `_on_apply_roi_settings` → ValidationManager (36 lines)
2. `_prepare_single_video_ui_state` → StateSynchronizer (36 lines)
3. `_check_live_project_calibration` → ValidationManager (31 lines)
