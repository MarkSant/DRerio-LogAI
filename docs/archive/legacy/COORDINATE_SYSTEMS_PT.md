...
**Dimensões:** Sempre 600xN pixels, onde N é calculado mantendo o aspect ratio real do aquário
...
**Dimensões:** Dimensões físicas informadas pelo usuário (ex: 54x24 cm)
...
```
┌─────────────────────────────────────────────────────────────────┐
│ 1. ESPAÇO ORIGINAL (1920x1080 px)                               │
│    - Frames capturados                                           │
...
└────────────────────────┬────────────────────────────────────────┘
                         ...
                         │
┌────────────────────────┴────────────────────────────────────────┐
│ 3. ESPAÇO EM CENTÍMETROS (54x24 cm)                             │
...
- Dimensões warped: 600px x 266px
...
while not self.cancel_event.is_set():
    ret, frame = cap.read()  # Frame ORIGINAL (1920x1080)
    ...
    for x1, y1, x2, y2, confidence, track_id in detections:
        ...
        if self.calibration:
            x1_w, y1_w, x2_w, y2_w = self.calibration.transform_bbox(
                x1, y1, x2, y2  # Original (1920x1080)
            )
            # Agora em warped (600x266)
...
```
ENTRADA (espaço original 1920x1080):
...
```python
center_original = (960, 540)  # Centro do frame 1920x1080
```

**Após transformação:**
```python
center_warped = cal.transform_point(960, 540)
# ≈ (300, 133) - Centro do warped 600x266

center_cm = (300/11.11, (266-133)/11.08)
# ≈ (27, 12) - Centro do aquário 54x24 cm ✅
```
...
4. **pixel_per_cm_ratio se refere ao espaço WARPED**
   - Sempre 600xN pixels → dimensões reais em cm
