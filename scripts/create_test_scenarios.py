#!/usr/bin/env python3
"""
Script para criar cenários de teste do Wizard v1.5

Gera estruturas de pastas com vídeos mock e parquets para testar:
1. Projeto experimental com design detectável
2. Projeto com parquets para importação
3. Projeto exploratório simples
...
Saída:
    ...
    └── scenario_3_exploratory/   (exploratório simples)
"""
...
def create_mock_video(output_path: Path, duration_seconds: float = 5, fps: int = 30):
    """
    Cria um vídeo mock com frames coloridos e texto.

    Args:
        output_path: Caminho para salvar o vídeo (.mp4)
...
def create_mock_parquet(video_path: Path, parquet_type: str = "trajectory"):
    """
    Cria um arquivo Parquet mock próximo ao vídeo.

    Args:
        video_path: Caminho do vídeo de referência
    ...
    if parquet_type == "arena":
        ...
        # Mock de trajetória (30 frames, 1 animal)
...
def create_scenario_1_experimental(base_dir: Path):
    """
    Cenário 1: Projeto Experimental com Design Detectável
...
def create_scenario_2_with_parquets(base_dir: Path):
    """
    Cenário 2: Projeto com Parquets Existentes para Importação
    ...
    O wizard deve detectar:
    - video1: arena + rois + trajectory → opção "Full" ou "Import Zones"
    - video2: arena + rois → opção "Import Zones"
    - video3: nenhum parquet → opção "Skip" ou processar normalmente
    """
    print("\n📁 CENÁRIO 2: Projeto com Parquets para Importação")
    print("=" * 60)

    scenario_dir = base_dir / "scenario_2_with_parquets"
    scenario_dir.mkdir(parents=True, exist_ok=True)

    # Video 1: Com arena, ROIs e trajetória
...
def create_scenario_3_exploratory(base_dir: Path):
    """
    Cenário 3: Projeto Exploratório Simples
    ...
    O wizard deve:
    - NÃO detectar design experimental (apenas datas)
    - Tipo de projeto: Exploratório
    """
    print("\n📁 CENÁRIO 3: Projeto Exploratório Simples")
    print("=" * 60)
...
def create_readme(base_dir: Path):
    """Cria README com instruções de teste."""
    readme_content = """# Test Scenarios para Wizard v1.5

Este diretório contém cenários de teste gerados automaticamente para o wizard.
...
**Vídeos**: 3 arquivos com datas nos nomes
...
- [ ] "Add Files" → dialog de seleção de arquivos
- [ ] "Add Folder" → dialog de seleção de pasta, scan recursivo funciona
- [ ] Remove vídeo da lista → vídeo removido
- [ ] Clear All → lista fica vazia
- [ ] Next sem vídeos → mensagem de erro
- [ ] Next com vídeos → prossegue para Detection

### Step 3: Detection
- [ ] Cenário 1: Design experimental detectado corretamente
- [ ] Cenário 2: Parquets detectados e contados corretamente
- [ ] Cenário 3: Nenhum design detectado (ou baixa confiança)
- [ ] Progress bar aparece durante scan
- [ ] Cancel interrompe o scan

### Step 4: Import Configuration (só aparece se há parquets)
- [ ] Cenário 2 video1: Opções "Skip", "Import Zones", "Partial", "Full"
- [ ] Cenário 2 video2: Opções "Skip", "Import Zones"
- [ ] Cenário 2 video3: Opções "Skip" (sem import)
- [ ] Alterar action de um vídeo → persiste ao navegar Back/Next
- [ ] ROI Merge Strategy: "Replace" vs "Merge" → tooltip explica

### Step 5: Confirmation
- [ ] Mostra resumo correto (nome, tipo, vídeos, design)
...
- **Resolução**: 640x480
- **FPS**: 30
- **Conteúdo**: Gradiente de cor + texto + "fish" simulado (círculo branco se movendo)

Cada parquet mock tem:
- **Arena**: Polígono retangular 640x480
