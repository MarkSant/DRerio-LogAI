...
class TestImportConfigStep:
    ...
    def test_import_config_step_builds_ui_without_error(self):
        ...
        assert step.step_id == WizardStepID.IMPORT_CONFIG
    ...
    def test_import_config_step_default_state_is_empty(self):
        ...
        assert step.video_configs == []
        merge_value = step.roi_merge_strategy_var.get()
        assert merge_value == ROIMergeStrategy.REPLACE.value
    ...
    def test_smart_defaults_import_scope_all(self):
        ...
        assert len(step.video_configs) == 1
        config = step.video_configs[0]

        assert config["import_arena"]
        assert config["import_rois"]
        assert config["import_trajectory"]
        assert config["action"] == ImportAction.SKIP.value
    ...
    def test_smart_defaults_import_scope_zones(self):
        ...
        assert config["import_arena"]
        assert config["import_rois"]
        assert not config["import_trajectory"]  # Never import trajectory
        assert config["action"] == ImportAction.IMPORT_ZONES.value
    ...
    def test_smart_defaults_import_scope_none(self):
        ...
        assert not config["import_arena"]
        assert not config["import_rois"]
        assert not config["import_trajectory"]
        assert config["action"] == ImportAction.FULL.value
    ...
    def test_smart_defaults_partial_parquets(self):
        ...
        assert config["import_arena"]
        assert not config["import_rois"]  # Not available
        assert not config["import_trajectory"]  # Not available
        assert config["action"] == ImportAction.PARTIAL.value
    ...
    def test_get_data_returns_clean_config(self):
        ...
        assert "import_config" in data
        assert "roi_merge_strategy" in data

        # Config should have exactly the expected fields
        config = data["import_config"][0]
        assert "video" in config
        assert "import_arena" in config
        assert "import_rois" in config
        assert "import_trajectory" in config
        assert "action" in config

        # Should NOT have internal fields
        assert "has_arena" not in config
        assert "has_rois" not in config
        assert "has_trajectory" not in config
    ...
    def test_roi_merge_strategy_selection(self):
        ...
        assert data["roi_merge_strategy"] == ROIMergeStrategy.MERGE.value
    ...
    def test_set_data_restores_ui(self):
        ...
        assert len(step.video_configs) == 1
        assert step.video_configs[0]["import_arena"]
        assert not step.video_configs[0]["import_rois"]
        merge_value = step.roi_merge_strategy_var.get()
    ...
    def test_summary_calculation(self):
        ...
        assert "1" in summary  # 1 SKIP
        assert "1" in summary  # 1 IMPORT_ZONES
        assert "1" in summary  # 1 FULL
