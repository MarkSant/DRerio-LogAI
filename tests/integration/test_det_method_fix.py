"""
Integration test that simulates the exact issue scenario mentioned in the problem statement.
This test validates that the "det" method for animal detection now works correctly.
"""
import os
import tempfile
...
def test_animal_detection_det_method_issue():
    """
    Test the specific issue: 'Falha ao iniciar o detector de "animal" no fluxo de Vídeo Único'
    when animal_method is "det" and weights.get_path.not_found is logged.
    
    This reproduces the exact scenario from the problem statement and validates the fix.
    """
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('zebtrack.core.weight_manager.settings', mock_settings):
            # Initialize WeightManager (this is what happens in controller.setup_detector())
            wm = WeightManager(config_dir=temp_dir)
            
            # This is the critical call that was failing:
            # controller.setup_detector() calls get_weight_path_by_method("det", "animal")
            # when animal_method = "det"
            det_path = wm.get_weight_path_by_method("det", "animal")
            
            # Before the fix, this would return None and log "weights.get_path.not_found"
            # After the fix, it should return the path to the detection weight
            assert det_path is not None, "Detection weight path should not be None after fix!"
            assert det_path == det_file, f"Expected det path {det_file}, got {det_path}"
...
def test_controller_setup_detector_scenario():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('zebtrack.core.weight_manager.settings', mock_settings):
            ...
            # Verify the fix prevents the original error condition
            # Original error: "Nenhum modelo det está disponível para detecção de animais."
            # This should not happen anymore because we found the detection model
