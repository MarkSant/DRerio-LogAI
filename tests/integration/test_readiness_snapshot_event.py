...
class TestReadinessSnapshotEvent:
    ...
    def test_event_bus_v2_publishes_readiness_snapshot(self):
        ...
        event_bus.publish(
            Event(type=UIEvents.READINESS_SNAPSHOT_UPDATED, data=snapshot_data, source="test")
        )
    ...
    def test_dialog_manager_publishes_readiness_snapshot_dual_mode(self):
        ...
        with patch(
            ...
            try:
                ...
                # If method fails due to missing dependencies, that's OK - we're testing
                # event publishing
                pass
    ...
    def test_dual_mode_removed_legacy_path_does_not_execute(self):
        ...
        # Assert
        assert gui_mock.apply_pending_readiness_snapshot.call_count == 0, (
            "Old path should NOT execute"
        )
    ...
    def test_multiple_videos_in_each_category(self):
        ...
        # Act
        snapshot_data = {
            "ready_with_trajectory": [{"path": "/video1.mp4"}, {"path": "/video2.mp4"}],
            "ready_with_zones": [
                {"path": "/video3.mp4"},
                {"path": "/video4.mp4"},
                {"path": "/video5.mp4"},
            ],
            "arena_only": [{"path": "/video6.mp4"}],
            "without_arena": [
                {"path": "/video7.mp4"},
                {"path": "/video8.mp4"},
                {"path": "/video9.mp4"},
