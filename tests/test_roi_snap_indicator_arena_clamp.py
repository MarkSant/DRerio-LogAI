...
def test_roi_snap_indicator_arena_clamp_implementation():
    ...
    assert 'if self.current_drawing_type == "roi":' in motion_section, \
        "Should check if drawing ROI"
    ...
    assert 'closest_point on the arena boundary' in motion_section or \
           'closest point on the arena boundary' in motion_section, \
    ...
    assert 'self.roi_canvas.create_oval(' in motion_section, \
        "Should create snap indicator oval"
...
def test_roi_vertex_editing_arena_clamp_implementation():
    ...
    assert 'def _on_handle_drag(self, event):' in gui_code, \
        "_on_handle_drag method should exist"
    ...
    assert 'if (' in drag_section and 'current_editing_zone' in drag_section, \
    ...
    assert 'closest_point on the arena boundary' in drag_section or \
           'closest point on the arena boundary' in drag_section, \
    ...
    assert 'is_on_boundary' in draw_poly_section, \
        "Should detect vertices on arena boundary"
