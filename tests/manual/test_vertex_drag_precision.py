...
def test_coordinate_conversion_precision():
    ...
    # Verify precision is maintained (error should be minimal due to float precision)
...
def test_smooth_drag_simulation():
    ...
    class MockGUI:
        def __init__(self):
            ...
            self.edited_polygon_points = [[100.0, 100.0], [200.0, 100.0], [200.0, 200.0], [100.0, 200.0]]
    ...
    for i in range(steps + 1):
        ...
        # Simulate drag event
        event = MockEvent(x, y)
        original_point = list(mock_gui.edited_polygon_points[0])
        ...
        if i > 0:
            ...
            if distance > 10.0:  # Arbitrary threshold for "jumping"
                print(f"❌ FALHA: Salto detectado no passo {i}: distância = {distance:.3f}")
                return False
...
def main():
    ...
    tests = [
        ("Precisão das conversões de coordenadas", test_coordinate_conversion_precision),
