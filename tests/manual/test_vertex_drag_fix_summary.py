...
def demo_fix_summary():
    ...
    print(
        f"   Precisão perdida: {abs(new_result[0] - old_result[0]):.3f}, "
        f"{abs(new_result[1] - old_result[1]):.3f}"
    )
    ...
    for i, (old_pos, new_pos) in enumerate(zip(old_positions, new_positions)):
        ...
        print(
            f"   {i:4d}  | ({old_pos[0]:6.1f}, {old_pos[1]:6.1f}) | "
            f"({new_pos[0]:6.1f}, {new_pos[1]:6.1f}) | "
            f"({diff_x:4.1f}, {diff_y:4.1f})"
        )

    final_old = old_positions[-1]
    final_new = new_positions[-1]
    total_error = (
        (final_new[0] - final_old[0]) ** 2 + (final_new[1] - final_old[1]) ** 2
    ) ** 0.5
