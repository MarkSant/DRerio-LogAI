...
# -*- coding: utf-8 -*-
"""
Validation test for the specific vertex drag issue described in the problem statement.
This test reproduces the "very little movement with each drag attempt" issue 
and validates that it's been fixed.
"""
...
def test_old_vs_new_behavior():
    ...
    for attempt in range(5):
        ...
        attempted_pos = (current_pos[0] + desired_move_x, current_pos[1] + desired_move_y)
        ...
        if abs(movement_x) < 0.1 and abs(movement_y) < 0.1:
            print(f"    ⚠️  Movimento muito pequeno detectado!")
    ...
    old_distance = ((final_old_pos[0] - target_canvas_pos[0])**2 + (final_old_pos[1] - target_canvas_pos[1])**2)**0.5
    new_distance = ((final_new_pos[0] - target_canvas_pos[0])**2 + (final_new_pos[1] - target_canvas_pos[1])**2)**0.5
