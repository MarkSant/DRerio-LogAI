...
class TestAnalysisServiceVideoProcessing:
    ...
    def test_load_trajectory_from_parquet_success(self, analysis_service, sample_trajectory_parquet):
    ...
    def test_run_analysis_pipeline_success(self, mock_roi_analyzer_class, mock_behavior_analyzer_class, analysis_service):
        ...
        mock_roi.get_roi_transitions.return_value = MagicMock(to_dict=lambda x: {})
        mock_roi.get_event_log.return_value = MagicMock(to_dict=lambda x: [])
        mock_roi_analyzer_class.return_value = mock_roi
        
        # Create sample data
        df = pd.DataFrame({
            'x1': [100, 110], 'y1': [100, 110],
            'x2': [150, 160], 'y2': [150, 160]
        })
        ...
        report, behavior_analyzer, roi_analyzer = result
    ...
    def test_resolve_analysis_profile_default(self, analysis_service, mock_settings):
        ...
        assert profile['freezing_vel_threshold'] == mock_settings.video_processing.freezing_velocity_threshold
    ...
    def test_profile_matches_exact_match(self, analysis_service):
        ...
        profile = {
            'criteria': {'group': 'control', 'subject': 'fish_1'}
        }
    ...
    def test_default_analysis_profile(self, analysis_service, mock_settings):
        ...
        assert profile['name'] == 'default'
        assert profile['freezing_vel_threshold'] == mock_settings.video_processing.freezing_velocity_threshold
        assert profile['smoothing_window_length'] == mock_settings.trajectory_smoothing.window_length
